...
// Check if Foundation/Intermediate/Advanced categories exist in the system
// Get all course categories to check for Foundation/Intermediate/Advanced
$has_foundation_categories = false;

// Get all category IDs from all courses
$category_sql = "SELECT DISTINCT c.category
                 FROM {course} c
                 WHERE c.id != 1
                 AND c.visible = 1";

$all_category_ids = $DB->get_fieldset_sql($category_sql);

// Check if any category names match Foundation, Intermediate, or Advanced
if (!empty($all_category_ids)) {
    list($cat_insql, $cat_params) = $DB->get_in_or_equal($all_category_ids, SQL_PARAMS_NAMED, 'cat');
...
$selected_region = optional_param('region', '', PARAM_TEXT);
...
// Parse region (single or array - GCC/KSA)
$selected_regions = [];
if (isset($_GET['regions'])) {
    if (is_array($_GET['regions'])) {
        $selected_regions = array_map('trim', $_GET['regions']);
    } else {
        $selected_regions = [trim($_GET['regions'])];
    }
...
// Region filter (GCC / KSA). Existing books without region are treated as GCC.
if (!empty($selected_regions)) {
    $has_filters = true;
    $region_parts = [];
    foreach ($selected_regions as $idx => $reg) {
        $reg = trim($reg);
        if ($reg === 'GCC') {
            $region_parts[] = "(region = 'GCC' OR region IS NULL OR region = '')";
        } elseif ($reg === 'KSA') {
            $region_parts[] = "region = 'KSA'";
        }
    }
    if (!empty($region_parts)) {
        $sql_conditions[] = "(" . implode(' OR ', $region_parts) . ")";
    }
}
...
// Require at least one region (GCC/KSA) to be selected before showing books
if (empty($selected_regions) && !$selected_region) {
    $has_filters = false;
}
        ...
        <div class="col-12">
            ...
            <div class="main-content">
                ...
                <div class="ebooks-filter-section">
                    ...
                    <div class="filter-cards-container" id="regionFilterCards">
                        <label class="filter-card region-card gcc-card <?php echo in_array('GCC', $selected_regions) || $selected_region == 'GCC' ? 'selected' : ''; ?>" data-region="GCC">
                            ...
                            <div class="level-card-icon-wrapper">
                                <div class="filter-card-icon level-icon" style="background: #dbeafe; color: #2563eb;">
                                    <i class="fa fa-globe"></i>
                                </div>
                            </div>
                            <div class="filter-card-content level-card-content">
                                <h4 class="filter-card-title">GCC</h4>
                        ...
                        <label class="filter-card region-card ksa-card <?php echo in_array('KSA', $selected_regions) || $selected_region == 'KSA' ? 'selected' : ''; ?>" data-region="KSA">
                            ...
                            <div class="filter-card-checkbox region-checkbox"></div>
                            <div class="level-card-icon-wrapper">
                                <div class="filter-card-icon level-icon" style="background: #fef3c7; color: #d97706;">
                                    <i class="fa fa-flag"></i>
                                </div>
                            </div>
                            <div class="filter-card-content level-card-content">
                                <h4 class="filter-card-title">KSA</h4>
                    ...
                    <div class="filter-section-wrapper" id="levelSectionWrapper" style="<?php echo (empty($selected_regions) && !$selected_region) ? 'display: none;' : ''; ?>">
    ...
    <div class="modal-content">
        ...
        <form id="bookForm" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label for="bookRegion">Curriculum / Region *</label>
                <select id="bookRegion" name="region" required>
                    <option value="GCC" <?php echo (!empty($selected_regions) && in_array('GCC', $selected_regions)) || $selected_region == 'GCC' ? 'selected' : ''; ?>>GCC</option>
                    <option value="KSA" <?php echo (!empty($selected_regions) && in_array('KSA', $selected_regions)) || $selected_region == 'KSA' ? 'selected' : ''; ?>>KSA</option>
                </select>
            </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function filterBooks(resetPage = false) {
        ...
        if (!booksSection) {
            ...
            const filterBoxes = document.querySelector('.filter-boxes');
            if (filterBoxes && filterBoxes.parentNode) {
                filterBoxes.parentNode.insertBefore(booksSection, filterBoxes.nextSibling);
            } else {
                document.querySelector('.main-content').appendChild(booksSection);
            }
        }
        ...
        document.querySelectorAll('.pagination-btn, .pagination-page').forEach(btn => {
            btn.addEventListener('click', function(e) {
                ...
                if (page && page > 0) {
                    ...
                    // Reload books for the new page (don't reset page)
                    filterBooks(false);
...
function editBook(bookId) {
    fetch('<?php echo $CFG->wwwroot; ?>/theme/remui_kids/admin/ajax/get_book.php?id=' + bookId)
        ...
        .then(data => {
            ...
            if (data.region) {
                document.getElementById('bookRegion').value = data.region;
            } else {
                document.getElementById('bookRegion').value = 'GCC';
            }
            if (data.cover_image) {
