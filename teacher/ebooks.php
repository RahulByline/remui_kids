...
// Parse region (GCC / KSA) - support both 'regions' and 'regions[]' (AJAX sends regions[]=GCC)
$selected_regions = [];
$regions_param = isset($_GET['regions']) ? $_GET['regions'] : (isset($_GET['regions[]']) ? $_GET['regions[]'] : null);
if ($regions_param !== null) {
    if (is_array($regions_param)) {
        $selected_regions = array_map('trim', $regions_param);
    } else {
        $selected_regions = [trim($regions_param)];
    }
...
// When region column doesn't exist yet (legacy), selecting GCC shows all books.
if (!empty($selected_regions)) {
    ...
    if ($region_column_exists) {
        ...
        // Legacy: no region column — treat GCC selection as "show all previously uploaded books"
        $sql_conditions[] = "1=1";
    }
}
